...
-- 3. 사용자 테이블에서 사용자번호가 5인 사용자를 삭제하시오. 사용자번호가 5인 사용자의 구매 내역을 먼저 삭제한 뒤 진행하시오.
-- 데이터 삭제는 DELETE / 참고: DROP-테이블 삭제, TRUNCATE-데이터 전체 삭제, DELETE-원하는 행 삭제
-- 해당 사용자 삭제시 구매 테이블에 있는 사용자 구매내역이 해당 사용자를 참조하고 있으므로 삭제 오류 발생, 
-- 따라서 구매 테이블의 내역에서 5번 사용자가 구매한 내역을 삭제한 후, 사용자 테이블에서 5번 사용자를 삭제해야 함
--1)
DELETE 
	FROM USERS 
	WHERE USER_NO = '5';
...
DELETE 
	...
	WHERE USER_ID = (SELECT USER_ID
					 FROM USERS 
					 WHERE USER_NO = 5); 
-- 1), 2)를 트랜잭션(TRANSACTION)으로 처리하려면 2)를 실행하고 1)실행한 후 모두 정상적으로 삭제가 됐을 때, 
-- COMMIT 처리를 하고, 두 가지 중 하나가 정상적으로 실행되지 못한 경우 ROLLBACK 처리 함  
...
-- KYM     1
-- LHJ     2
SELECT USER_ID AS 아이디, COUNT(*) 구매횟수
	FROM BUYS 
	GROUP BY USER_ID  		-- GROUP BY에는 별칭을 사용할 수 없음 / 무조건 컬럼명
	ORDER BY 구매횟수 DESC;		-- ORDER BY에는 별칭 사용 가능, 기본정렬 ASC(오름차순)
...
-- KHD     강호동    3         1210
-- PSH     박수홍    3         1860
-- GROUP BY에 쓰인 컬럼은 SELECT에 무조건 적어줘야 함 / GROUP BY에 있는 컬럼들은 SELECT에서 생략 가능
SELECT U.USER_ID , U.USER_NAME , 
COUNT(*) 구매횟수, SUM(P.PROD_PRICE*B.BUY_AMOUNT) 총구매액
	FROM USERS u INNER JOIN BUYS b
	ON U.USER_ID = B.USER_ID INNER JOIN PRODUCTS p 
	ON B.PROD_CODE = P.PROD_CODE 
	GROUP BY U.USER_ID, U.USER_NAME
	HAVING SUM(P.PROD_PRICE*B.BUY_AMOUNT) >= 1000; 
--GROUP BY절을 사용한 경우, 조건을 설정하려면 WHERE가 아닌 HAVING을 사용한다. 이때 HAVING 절에도 별칭을 사용할 수 없음

-- 8. 구매 이력과 상관 없이 고객별 구매횟수를 조회하시오.(OUTER JOIN)
...
-- SDY     신동엽    0
-- YJS     유재석    0
SELECT U.USER_ID 아이디, U.USER_NAME 고객명, COUNT(B.BUY_NO) 구매횟수
...
-- 박수홍  메모리  800
-- 박수홍  모니터  1000
SELECT U.USER_NAME 고객명, P.PROD_NAME 제품명, P.PROD_PRICE*B.BUY_AMOUNT  구매액 
	FROM USERS u INNER JOIN BUYS b 
	ON U.USER_ID = B.USER_ID INNER JOIN PRODUCTS p 
	ON B.PROD_CODE = P.PROD_CODE 
...
-- 이휘재  2
-- 강호동  3
SELECT U.USER_ID 아이디,U.USER_NAME 고객명, COUNT(*) 구매횟수
	FROM USERS u INNER JOIN BUYS b 
	ON U.USER_ID = B.USER_ID 
	GROUP BY U.USER_ID, U.USER_NAME	 	--중복되는 이름이 있을 수 있기 때문에
	HAVING COUNT(*) >= 2;
	
-- 11. 모든 제품의 제품명과 판매횟수를 조회하시오.
--     판매 이력이 없는 제품은 0으로 조회하시오.(OUTER JOIN)
-- 제품명  판매횟수
-- 책      1
-- 메모리    1
-- 운동화    2
...
-- 청바지    2 
-- 노트북    1
SELECT P.PROD_NAME 제품명, COUNT(B.BUY_NO) 구매횟수
...
-- 신동엽   NULL
-- 유재석   NULL
SELECT U.USER_NAME 고객명,P.PROD_NAME 구매제품
	FROM USERS u LEFT OUTER JOIN BUYS b 
	ON U.USER_ID = B.USER_ID LEFT OUTER JOIN PRODUCTS p 
	ON B.PROD_CODE = P.PROD_CODE 
	WHERE B.PROD_CODE IS NOT NULL 
	ORDER BY U.USER_ID;
	
-- 스칼라 서브쿼리 이용
SELECT U.USER_NAME 고객명, 
	(SELECT P.PROD_NAME
	 FROM PRODUCTS p
	 WHERE P.PROD_CODE = B.PROD_CODE) 구매제품
	FROM USERS u LEFT OUTER JOIN BUYS b 
	ON U.USER_ID = B.USER_ID
	WHERE B.PROD_CODE IS NOT NULL 
	ORDER BY U.USER_ID;