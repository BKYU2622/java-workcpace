...
--  2) 회원가입일: REGIST_DATE DATE / DEFAULT SYSDATE: 기본값을 SYSDATE로 준다는 뜻
ALTER TABLE MEMBER_TBL ADD ADDRESS VARCHAR2(200);
-- 가입일을 입력 안 했을 경우, 기본값으로 현재 날짜가 입력 됨
ALTER TABLE MEMBER_TBL ADD REGIST_DATE DATE DEFAULT SYSDATE; 

-- 3. 추가된 회원주소 칼럼을 다시 삭제하시오.
ALTER TABLE MEMBER_TBL DROP COLUMN ADDRESS;

-- 4. 회원등급 칼럼의 타입을 VARCHAR2(20 BYTE)으로 수정하시오.
ALTER TABLE MEMBER_TBL MODIFY GRADE VARCHAR2(20);
-- 5. 회원패스워드 칼럼의 이름을 PWD로 수정하시오.
ALTER TABLE MEMBER_TBL RENAME COLUMN PW TO PWD;

-- 6. 회원번호 칼럼에 기본키(PK_MEMBER)를 설정하시오.
ALTER TABLE MEMBER_TBL ADD CONSTRAINT PK_MEMBER_TBL PRIMARY KEY(MEMBER_NO);
...
-- 9. 글내용 칼럼의 필수 제약조건을 제거하시오. / 필수 제약조건 제거: NULL로 바꾸라는 의미
ALTER TABLE BOARD_TBL MODIFY CONTENT VARCHAR2(4000 BYTE) NULL; 

-- 10. 글번호 칼럼에 기본키(PK_BOARD)를 설정하시오.
ALTER TABLE BOARD_TBL ADD CONSTRAINT PK_BOARD_TBL PRIMARY KEY(BOARD_NO);
-- 제약조건 확인
SELECT OWNER, CONSTRAINT_NAME, CONSTRAINT_TYPE, STATUS
	FROM USER_CONSTRAINTS 
	WHERE TABLE_NAME = 'BOARD_TBL';

-- 11. 작성자 칼럼에 MEMBER_TBL 테이블의 회원아이디를 참조하는 FK_BOARD_MEMBER 외래키를 설정하시오.
-- 게시글을 작성한 회원 정보가 삭제되면 해당 회원이 작성한 게시글도 모두 함께 지워지도록 처리하시오.
ALTER TABLE BOARD_TBL ADD CONSTRAINT FK_BOARD_MEMBER 
FOREIGN KEY(WRITER)	REFERENCES MEMBER_TBL(ID) ON DELETE CASCADE;
