...
INSERT INTO BOOK_TBL(BOOK_ID,BOOK_NAME,PUBLISHER,PRICE) VALUES (BOOK_SEQ.NEXTVAL,'역도의 단계별 기술','굿스포츠',6000);
...
INSERT INTO CUSTOMER_TBL  VALUES (CUSTOMER_SEQ.NEXTVAL,'박지성','영국','000-000-000');
INSERT INTO CUSTOMER_TBL  VALUES (CUSTOMER_SEQ.NEXTVAL,'김연아','대한민국','111-111-111');
INSERT INTO CUSTOMER_TBL  VALUES (CUSTOMER_SEQ.NEXTVAL,'장미란','대한민국','222-222-222');
INSERT INTO CUSTOMER_TBL  VALUES (CUSTOMER_SEQ.NEXTVAL,'추신수','미국','333-333-333');
INSERT INTO CUSTOMER_TBL  VALUES (CUSTOMER_SEQ.NEXTVAL,'박세리','대한민국',NULL);
SELECT * FROM CUSTOMER_TBL;

-- 4. 1부터 1씩 증가하는 ORDER_SEQ 시퀀스를 생성한 뒤, '현재날짜6자리-시퀀스' 형식으로 주문번호를 만들어서 INSERT를 수행하시오.
-- INSERT할 때 구매번호 생성방법 : TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL
/*
...
--DROP TABLE ORDER_TBL;
INSERT INTO ORDER_TBL VALUES ( TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL,1000,1,1,'20/07/01');
INSERT INTO ORDER_TBL VALUES ( TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL,1000,3,2,'20/07/03');
INSERT INTO ORDER_TBL VALUES ( TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL,1001,5,1,'20/07/03');
INSERT INTO ORDER_TBL VALUES ( TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL,1002,6,2,'20/07/04');
INSERT INTO ORDER_TBL VALUES ( TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL,1003,7,3,'20/07/05');
INSERT INTO ORDER_TBL VALUES ( TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL,1000,2,5,'20/07/07');
INSERT INTO ORDER_TBL VALUES ( TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL,1003,8,2,'20/07/07');
INSERT INTO ORDER_TBL VALUES ( TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL,1002,10,2,'20/07/08');
INSERT INTO ORDER_TBL VALUES ( TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL,1001,10,1,'20/07/09');
INSERT INTO ORDER_TBL VALUES ( TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL,1002,6,4,'2020/07/10');
SELECT * FROM ORDER_TBL;
ALTER SESSION SET NLS_DATE_FORMAT='YYYY-MM-DD';

SELECT TO_CHAR(ORDER_DATE,'YYYY-MM-DD') FROM ORDER_TBL;
DELETE FROM ORDER_TBL WHERE ORDER_ID = '230420-21';
...
ALTER TABLE ORDER_TBL ADD CONSTRAINT FK_ORDER_CUSTOMER FOREIGN KEY(CUSTOMER_ID) 
REFERENCES CUSTOMER_TBL(CUSTOMER_ID) ON DELETE SET NULL; 
ALTER TABLE ORDER_TBL ADD CONSTRAINT FK_ORDER_BOOK FOREIGN KEY(BOOK_ID) 
REFERENCES BOOK_TBL(BOOK_ID) ON DELETE SET NULL; 
...
-- 주문한 이력이 없는 고객이름 출력(GROUP BY에 대한 조건으로 HAVING을 사용) 
...
-- 고객명  출판사수
-- 박지성  3
SELECT C.CUSTOMER_NAME 고객명, COUNT(DISTINCT B.PUBLISHER) 출판사수
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
...
SELECT O.ORDER_ID 구매번호, C.CUSTOMER_NAME 구매자, 
       B.BOOK_NAME 책이름, B.PRICE 판매가격, O.ORDER_DATE 주문일자
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
    ON O.BOOK_ID = B.BOOK_ID
 WHERE TO_DATE(O.ORDER_DATE,'YY/MM/DD') 
       NOT BETWEEN TO_DATE('20/07/04','YY/MM/DD') AND TO_DATE('20/07/07','YY/MM/DD');
...
-- 박지성  116000
-- 추신수  86000
SELECT C.CUSTOMER_NAME 고객명, SUM(B.PRICE * O.AMOUNT) 총구매액
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
    ON O.BOOK_ID = B.BOOK_ID
 GROUP BY C.CUSTOMER_ID, C.CUSTOMER_NAME;
...
--LEFT OUTERE JOIN이라 왼쪽에 B.BOOK_ID가 와야함
-- 고객명  총구매액  구매횟수
-- 박지성  116000     3
-- 김연아  19000      2
-- 장미란  62000      3
...
SELECT C.CUSTOMER_NAME 고객명,
       NVL(SUM(B.PRICE * O.AMOUNT),0) 총구매액,
       COUNT(O.ORDER_ID) 구매횟수
  FROM CUSTOMER_TBL C LEFT OUTER JOIN ORDER_TBL O
...
-- 고객명  책이름       책가격
-- NULL    골프 바이블  35000
SELECT NVL(C.CUSTOMER_NAME,'주문고객없음') 고객명, B.BOOK_NAME 책이름, B.PRICE 책가격
...
-- 15. 총구매액이 2~3위인 고객의 이름과 총구매액을 조회하시오(ROWNUM)
-- 고객명  총구매액
-- 추신수  86000
-- 장미란  62000
--총 구매액을 내림차순(DESC)으로 정렬한 후 ROWNUM이 2~3인 고객 조회
                    
-- ROWNUM을 이용한 경우
SELECT BB.고객명, BB.총구매액
  FROM (  SELECT ROWNUM AS RNUM, AA.고객명, AA.총구매액
            FROM ( SELECT C.CUSTOMER_NAME 고객명, SUM(B.PRICE * O.AMOUNT) 총구매액
                     FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
                       ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
                       ON B.BOOK_ID = O.BOOK_ID
                    GROUP BY C.CUSTOMER_ID, C.CUSTOMER_NAME
                    ORDER BY 총구매액 DESC ) AA ) BB 
 WHERE BB.RNUM BETWEEN 2 AND 3;

-- ROW_NUMBER() 함수를 이용한 경우
SELECT A.고객명, A.총구매액
  FROM ( SELECT ROW_NUMBER() OVER(ORDER BY SUM(B.PRICE * O.AMOUNT) DESC ) AS RNUM
