...
public class Test {
	...
	public static void main(String[] args) {
		/*
		   JDBC(JAVA DATABASE CONNECTIVITY): 자바에서 데이터베이스에 접속할 수 있도록 하는 자바API이다.
		   JDBC용 객체
		    - DriverManager: Connection 객체를 생성하기 위한 객체
		    - Connection: DB에 접속된 객체(DB의 연결 정보를 담고 있는 객체)
		    - [Prepared]Statement: 연결된 DB에 sql문을 전달해서 실행하고 그 결과를 받아내는 객체 
		    - ResultSet: Select문 실행 후 조회된 결과를 담기위한 객체 
		    			 INSERT, UPDATE, DELETE문(DML)은 실행 후 반환값이 int값이 됨. 즉 영향을 받은 행의 수 
		   	...
		   	* JDBC 연결 과정(순서 중요)			 
		   	 ...
		   	 5) 결과 받기 
		   	   ...
		   	   > DML문(INSERT, UPDATE, DELETE) => int(처리된 행의 수)	=> 6-2)		 
		...
		// DB에 전달할 SQL문 작성 / sql문의 마지막 ;(새미콜론)은 포함하면 안 됨
		String sql = "SELECT TNO, TNAME, TDATE FROM TEST";
		
		try {
			
			// 1) JDBC DRIVER 등록
			// ojdbc6.jar 파일이 등록 안 됐거나, 오타가 있을 경우 => ClassNotFoundException 예외 발생
			...
			// 4), 5) sql문을 전달하면서 실행 후 결과(조회된 데이터)를 ResultSet 객체에 받기 
			rset = stmt.executeQuery(sql);
			
			// 6) ResultSet에 담겨있는 데이터들을 하나씩 추출 / 변수의 타입별로 get해줘야 함
			//	  ResultSet의 next() 메서드는 조회된 다음 레코드가 있는지 확인해서 있으면 true, 없으면 false반환
			while(rset.next()) {	// 여러 개 일때는 while문으로 출력, 하나이면 if문으로 출력
				
				// rset에 담겨있는 컬럼을 변수에 출력
				tno   = rset.getInt("tno");
